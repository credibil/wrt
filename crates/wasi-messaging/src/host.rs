...
impl HostMessage for Host<'_> {
    ...
    /// Optional metadata (also called metadata or attributes in some systems)
    ...
    async fn metadata(&mut self, self_: Resource<Message>) -> anyhow::Result<Option<Metadata>> {
        ...
        let metadata = msg
            .metadata
            .as_ref()
            .map(|h| h.iter().map(|(k, v)| (k.to_string(), v.to_string())).collect::<Vec<_>>());
    ...
    async fn add_metadata(
        ...
        metadata.insert(key.clone(), values[0].clone());
    ...
    async fn set_metadata(
        ...
        for (k, v) in &meta {
            ...
            metadata.insert(k.clone(), values[0].clone());
            for v in values.iter().skip(1) {
    ...
    /// set of metadata. So this function will copy the existing metadata,
